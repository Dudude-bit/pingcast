...
export async function generateMetadata({
  ...
  const c = dict.seo_software;
  return {
    title: c.metaTitle,
    description: c.metaDesc,
    alternates: {
      canonical: `/${lang}/status-page-software`,
      languages: Object.fromEntries(
        SUPPORTED_LOCALES.map((l) => [l, `/${l}/status-page-software`]),
      ),
    },
  };
}

export default async function StatusPageSoftwarePage({
  params,
}: {
  params: Params;
}) {
    ...
    <div className="container mx-auto px-4 py-12 max-w-3xl">
      <BreadcrumbListJsonLd
        items={[
          ...
          { name: c.crumb, url: `/${lang}/status-page-software` },
        ]}
      />
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        ...
        >
          {c.cta_primary} <ArrowRight className="ml-2 h-4 w-4" />
        </Link>
        <Link
          href={`/${lang}/pricing`}
      ...
      <section className="mt-12">
        ...
        <p className="mt-3 text-muted-foreground leading-relaxed">{c.features_intro}</p>
      ...
      <section className="mt-12">
        <h2 className="text-2xl font-bold tracking-tight">{c.h2_price}</h2>
        <p className="mt-3 text-muted-foreground leading-relaxed">{c.price_body}</p>
        <div className="mt-6">
          <Link
      ...
      <section className="mt-12">
        <h2 className="text-2xl font-bold tracking-tight">{c.h2_skip}</h2>
        <p className="mt-3 text-muted-foreground leading-relaxed">{c.skip_body}</p>
      </section>
