    ...
    <div className="container mx-auto px-4">
      ...
      <section className="border-y border-border/60 bg-muted/30">
        <div className="container mx-auto max-w-5xl py-8 px-4 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          <Stat label={l.stat_freq_label} value="30s" hint={l.stat_freq_hint} />
          <Stat label={l.stat_alert_label} value="< 10s" hint={l.stat_alert_hint} />
          <Stat label="Open source" value="MIT" hint={l.stat_oss_hint} />
          <Stat label={l.stat_stack_label} value="Go + Postgres" hint={l.stat_stack_hint} />
        </div>
      </section>
      ...
      <section className="py-20 max-w-5xl mx-auto">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          <StepCard n="01" icon={<Rocket className="h-5 w-5" />} title={l.step_register_title} body={l.step_register_body} />
          <StepCard n="02" icon={<Radio className="h-5 w-5" />} title={l.step_monitor_title} body={l.step_monitor_body} />
          <StepCard n="03" icon={<Plug className="h-5 w-5" />} title={l.step_channel_title} body={l.step_channel_body} />
        </div>
      </section>
      ...
      >
        <FeatureCard icon={<Globe className="h-6 w-6" />} title={l.feature_status_page_title} body={l.feature_status_page_body} />
        <FeatureCard icon={<LineChart className="h-6 w-6" />} title={l.feature_uptime_title} body={l.feature_uptime_body} />
        <FeatureCard icon={<Plug className="h-6 w-6" />} title={l.feature_atlassian_title} body={l.feature_atlassian_body} />
        <FeatureCard icon={<Zap className="h-6 w-6" />} title={l.feature_widget_title} body={l.feature_widget_body} />
        <FeatureCard icon={<Server className="h-6 w-6" />} title={l.feature_self_host_title} body={l.feature_self_host_body} />
        <FeatureCard icon={<GitBranch className="h-6 w-6" />} title={l.feature_pro_title} body={l.feature_pro_body} />
      </section>
        ...
        <div className="mt-10 overflow-x-auto rounded-xl border border-border/60 bg-card">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                <th className="text-left font-medium px-4 py-3 w-1/3">{l.compare_col_feature}</th>
                <th className="text-left font-medium px-4 py-3">PingCast</th>
                <th className="text-left font-medium px-4 py-3">Atlassian Statuspage</th>
            ...
            <tbody className="divide-y divide-border/50">
              <CompareRow label={l.compare_row_price} values={[l.compare_us_price, "$29/mo", "$20/mo", "$30/mo"]} />
              <CompareRow label={l.compare_row_custom_domain} values={[true, true, true, true]} />
              <CompareRow label={l.compare_row_branding} values={[true, true, true, l.compare_limited]} />
              <CompareRow label={l.compare_row_uptime} values={[true, false, false, true]} />
              <CompareRow label={l.compare_row_import} values={[true, l.compare_na, false, false]} />
              <CompareRow label={l.compare_row_widget} values={[true, false, false, false]} />
              <CompareRow label={l.compare_row_ru} values={[true, false, true, true]} />
              <CompareRow label={l.compare_row_self_host} values={[true, false, false, true]} />
              <CompareRow label={l.compare_row_oss} values={["MIT", false, false, "AGPL"]} />
            </tbody>
          </table>
        </div>
        <p className="mt-4 text-xs text-center text-muted-foreground">
          {l.compare_caption_pre}
          <Link href={`/${locale}/import/atlassian`} className="underline">
            {l.compare_caption_link}
          </Link>
          {l.compare_caption_post}
        </p>
      </section>
      ...
      <section className="py-16 max-w-4xl mx-auto">
        ...
        >
          ...
          <pre className="overflow-x-auto px-6 py-5 text-[13px] leading-relaxed font-mono">
            <code>
              <span className="text-muted-foreground">{`# ${l.code_comment}`}</span>
              {"\n"}
      ...
      <section className="py-16 max-w-3xl mx-auto">
        ...
        <FaqPageJsonLd items={l.faq_items as { q: string; a: string }[]} />
      </section>
...
function StepCard({
  ...
  return (
    ...
    >
      <span className="absolute right-4 top-4 font-mono text-xs text-muted-foreground/60">{n}</span>
  ...
  return (
    <tr>
      ...
      {values.map((v, i) => (
        <td
          key={i}
          className={`px-4 py-3 ${i === 0 ? "text-foreground" : "text-muted-foreground"}`}
        >
          {v === true ? (
